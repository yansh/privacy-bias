...
def create_dir_if_doesnt_exist(
    path_to_dir: Path, log: logging.Logger
) -> Optional[Path]:
    ...
    if not resolved_path.exists():
        ...
        try:
            ...
            log.error(
                "Error occurred while creating directory {}. Exception: {}".format(
                    resolved_path, e
                )
            )
            return None
...
def process_and_save_results(
    ...
    dataset_folder = args.dataset + (
        "pvar" if False else ""
    )  # Change flag for pvar mode
    os.makedirs(
        results_folder + args.model.split("/")[0] + "/" + dataset_folder, exist_ok=True
    )

    # Format temperature string
    temp = str(args.temperature).replace(".", "").replace("00", "0").replace("10", "1")
    filename = (
        f"{args.model.split('/')[1]}_{dataset_folder}_t{temp}_{args.prompt_type}.csv"
    )
    file_path = os.path.join(
        results_folder, args.model.split("/")[0], dataset_folder, filename
    )
...
def process_prompts(args, log):
    ...
    sampling_params = SamplingParams(
        max_tokens=100, temperature=args.temperature
    )  # , top_p=0.95)
    model = args.model
    ...
    for i, scenario in tqdm(
        zip(range(len(prompt_list[:sample_size])), prompt_list[:sample_size]),
        ...
        if args.prompt_type == "std":
            ...
            for j, order in zip(
                range(len(k_shot_prompts)),
                random.sample(list(likert_str.keys()), len(k_shot_prompts)),
            ):
        ...
        if args.prompt_type == "kshot":
            for i in range(len(k_shot_prompts)):
                if likert_random:
                    for j, order in zip(
                        range(3), random.sample(list(likert_str.keys()), 3)
                    ):
